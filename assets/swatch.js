...
//                 if (typeof index === 'undefined') return;
    ...
    function updateSticky_atc(variant,sticky_atc_js,js_sticky_sl,js_fgr_img,sticky_atc_price) {
        ...
        // Update Dimensions
        const $dimensionWrapper = $('[data-js-pdp-dimension]');
        if ($dimensionWrapper) {
          const $dimensionText = $dimensionWrapper.find('[data-js-pdp-dimension-text]')
          const text = $('[name="id"] option:selected').data('dimension')
          if (text) {
            $dimensionText.html(text);
            $dimensionWrapper.removeClass('hide')
          } else {
            $dimensionWrapper.addClass('hide')
          }
        }

        // Update Download link
        const $downloadLink = $('[data-js-pdp-download]');
        if ($downloadLink) {
          const link = $('[name="id"] option:selected').data('downloadLink')
          if (link) {
